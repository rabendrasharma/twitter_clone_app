class TweeetsController < ApplicationController
  ...
  def index
    ...
    @tweeet = Tweeet.new
    @users = User.all.where(:id.ne => current_user.id) if current_user.present?
  end
  ...
  def search 
    tweeets = []
    tweeets << User.any_of({ :name => /.*#{params[:search]}.*/i}, { :username => /.*#{params[:search]}.*/i}).map(&:tweeets)
    tweeets << Tweeet.any_of({ :tweeet => /.*#{params[:search]}.*/i})
